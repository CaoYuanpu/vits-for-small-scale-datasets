...
#                                    nn.BatchNorm3d(all_head_dim))
    ...
    def forward(self, x):
        ...
        print('split', q.shape, k.shape, v.shape)
        input()
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, split=False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        if split:
            self.attn = Attention_split(
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=384, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, split=False, **kwargs):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, split=split)
