...
best_acc1 = 0
MODELS = ['vit', 'vit_lora', 'vit_lora_split', 'swin' , 'cait']
...
def mark_only_lora_as_trainable(model: nn.Module, bias: str = 'none') -> None:
    ...
    for n, p in model.named_parameters():
        if 'norm' in n or ('embed'  in n and 'bias' not in n) or 'token' in n:
...
def main(args):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        name = full_rank_names[(2*epoch) % len(full_rank_names)]
        weight = full_rank_dict[name+'.weight']
        lora_A = full_rank_dict[name+'.lora_A']
        lora_B = full_rank_dict[name+'.lora_B']

        name2 = full_rank_names[(2*epoch+1) % len(full_rank_names)]
        weight2 = full_rank_dict[name2+'.weight']
        lora_A2 = full_rank_dict[name2+'.lora_A']
        lora_B2 = full_rank_dict[name2+'.lora_B']

        print(f'Full-rank training: {name} {name2}')
        weight.requires_grad = True
        lora_A.requires_grad = False
        lora_B.requires_grad = False

        weight2.requires_grad = True
        lora_A2.requires_grad = False
        lora_B2.requires_grad = False
        ...
        lora_A2.requires_grad = True
        lora_B2.requires_grad = True
...
if __name__ == '__main__':
    ...
    model_name += f"-{args.tag}-{args.dataset}-LR[{args.lr}-WD{args.weight_decay}]-Seed{args.seed}-Bias{args.bias}-OptCosAnnealWR-Reset{args.lora_reset}_partialsep2"
